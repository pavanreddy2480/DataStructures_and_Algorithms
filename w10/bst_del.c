...
void delete (nodeptr *root, int m) // to delete an element from the tree.
{
    nodeptr curptr;
    curptr = NULL;

    curptr = search(&(*root), m); // search for the element.
    ...
    else
    {
        nodeptr disp;
        disp = curptr;
        if (curptr->left == NULL && curptr->right == NULL) // both are null.
        {
            if (disp->parent->key < m)
            {
                disp->parent->right = NULL;
            }
            else
            {
                disp->parent->left = NULL;
            }
            free(disp);
            return;
        }
        else if (curptr->left == NULL || curptr->right == NULL) // one is null.
        {
            if (curptr->left == NULL)
            {
                if (curptr == *root)
                ...
                {
                    curptr->parent->left = curptr->right;
                    curptr->right->parent = curptr->parent;
                    free(disp);
                    return;
                }
                else
                {
                    curptr->parent->right = curptr->right;
                    curptr->right->parent = curptr->parent;
                    free(disp);
                    return;
                }
            ...
            else if (curptr->right == NULL)
            {
                ...
                {
                    curptr->parent->left = curptr->left;
                    curptr->left->parent = curptr->parent;
                    free(disp);
                    return;
                }
                else
                {
                    curptr->parent->right = curptr->left;
                    curptr->left->parent = curptr->parent;
                    free(disp);
                    return;
                }
        ...
        else if (curptr->left != NULL && curptr->right != NULL) // both are not null.
        {
            int c;
            nodeptr succ;
            succ = findpre(&(*root), m);
            ...
            delete (&(*root), (succ->key)); // calling delete recursively.
            curptr->key = c;
        }
    }
}

void check(int n) // used to print the statements.
{
    if (n == 2)
    {
